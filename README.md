...
- **Two menu shapes**: the full tree, or a short one that puts every available source a single click away
...
If walking that tree with the mouse gets awkward, the **ACE menu** setting has a **Short** shape that collapses all of it into one list:

```
Cam on Galaxy
├── OpCam  (2)
├── Available  (4)
│   ├── ANDERSON  -  MK41 VLS OPERATOR  -  Mk41 VLS  -  671 m
│   ├── BIANCHI  -  GUNNER  -  Slammer TUSK  -  120 m
│   ├── UGV Stomper  -  143 m
...
Categories and vehicle-type branches are skipped, the vehicle name moves into the line, and every source is one click from Cam on Galaxy. Helmet cams keep their own OpCam branch in both shapes.
...
**ACE menu** — **Normal**, the default, or **Short**. Normal is the tree shown above. Short puts every source under a single *Available* entry, one click from Cam on Galaxy. It is there because the ACE self-interaction menu opens as a wheel: the deeper the tree, the further the branches sit from the cursor, and on a busy mission the last level could close before the mouse reached it.
